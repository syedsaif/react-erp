...
export default function GenderForm({ initialData = {}, onSuccess, onCancel }) {
    ...
    async (data) => {
      try {
        ...
        if (data.id && data.id !== 0) {
          response = await api.put(`gender/Update`, payload);
          toast.success(response.data.Message || "✅ Gender updated successfully!");
        } else {
          response = await api.post(`gender/Insert`, payload);
          toast.success(response.data.message || "✅ Gender created successfully!");
        }
  ...
  return (
    <div className="form-section border p-4 mb-3 bg-light rounded">
      <h5>{formData.id && formData.id !== 0 ? "✏️ Edit Gender" : "➕ Add Gender"}</h5>
        ...
        <div className="row g-3">
          <div className="col-md-6">
            ...
            <input
              type="text"
              name="name"
              className="form-control"  // NO is-invalid here, so no red border
              value={formData.name}
