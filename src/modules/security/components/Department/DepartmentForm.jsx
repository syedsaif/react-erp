...
export default function DepartmentForm({ initialData = {}, onSuccess, onCancel }) {
  ...
  // ✅ Use ref to prevent duplicate toast for validation
  ...
  const { formData, errors, handleChange, handleSubmit, loading } = useForm(
    {
      ...
      companyId: userData.UserCompanyID ?? 1,
      createdBy: userData.UserAutoGeneratedId ?? 1,
      updatedBy: userData.UserAutoGeneratedId ?? 1,
      ip: "192.168.1.1",
    },
    validate,
    async (data) => {
      try {
        ...
        const payload = {
          ...
          CreatedBy: data.id ? undefined : data.createdBy,
          UpdatedBy: data.id ? data.updatedBy : undefined,
          Ip: data.ip,
        };
        ...
        if (data.id && data.id !== 0) {
          response = await axios.put(`${apiUrl}department/Update`, payload);
          toast.success(response.data.Message || "✅ Department updated successfully!");
        } else {
          response = await axios.post(`${apiUrl}department/Insert`, payload);
