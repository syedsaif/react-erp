...
export default function DepartmentForm({ initialData = {}, onSuccess, onCancel }) {
    ...
    async (data) => {
      try {
        ...
        if (data.id && data.id !== 0) {
          response = await api.put(`department/Update`, payload);
          toast.success(response.data.Message || "✅ Department updated successfully!");
        } else {
          response = await api.post(`department/Insert`, payload);
  ...
  return (
    <div className="form-section border p-4 mb-3 bg-light rounded">
      <h5>{formData.id && formData.id !== 0 ? "✏️ Edit Department" : "➕ Add Department"}</h5>
